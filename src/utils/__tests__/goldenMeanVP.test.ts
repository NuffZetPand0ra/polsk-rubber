...
describe('goldenMeanVP', () => {
    it('matches official Danish VP table for 20 random board/margin pairs (warn on soft mismatch)', () => {
      ...
      // Pick 20 random (boards, margin) pairs
      const tested = new Set<string>();
      for (let i = 0; i < 20; i++) {
        let boards, margin;
        do {
          boards = 1 + Math.floor(Math.random() * 10); // 1-10 boards for test (expand if needed)
          const maxMargin = table[boards].length - 1;
          margin = Math.floor(Math.random() * (maxMargin + 1));
        } while (tested.has(`${boards},${margin}`));
        tested.add(`${boards},${margin}`);
